...
public class DiscountCodeImpl implements DiscountCodeService {
    ...
    public DiscountCodeDto saveDiscountCode(DiscountCodeDto discountCodeDto) {
       if (discountCodeRepository.existsByCode(discountCodeDto.getCode())) {
            throw new ShopApiException(HttpStatus.BAD_REQUEST, "Tên giảm giá "+discountCodeDto.getCode()+" đã tồn tại");
        }
    ...
    public DiscountCodeDto getDiscountCodeById(Long id) {
        DiscountCode discountCode = discountCodeRepository.findById(id).orElseThrow(() -> new NotFoundException("Không tìm thấy tên giảm giá"));
    ...
    public DiscountCodeDto updateStatus(Long discountCodeId, int status) {
        DiscountCode discountCode = discountCodeRepository.findById(discountCodeId).orElseThrow(() -> new NotFoundException("Không tìm thấy tên giảm giá"));
