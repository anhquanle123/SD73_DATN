...
/*
 @GetMapping("/oauth2-success")
    public String oauth2LoginSuccess(OAuth2AuthenticationToken authentication, RedirectAttributes redirectAttributes) {
        ...
        if (account == null) {
            // Nếu chưa tồn tại tài khoản → tạo mới
            account = new Account();
            account.setEmail(email);
            account.setPassword(""); // Google không dùng mật khẩu

            Role role = new Role();
            role.setId(3L); // ROLE_USER
            account.setRole(role);
            account.setNonLocked(true);
            account.setCreateDate(LocalDateTime.now());
            ...
            customer.setPhoneNumber("N/A"); // Vì không có sdt từ Google
